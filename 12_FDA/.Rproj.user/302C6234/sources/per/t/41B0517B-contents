...
# B-spline (Basis spline) — это набор базисных функций,
# с помощью которых можно аппроксимировать (приближать) другие функции или данные.

# Вместо того, чтобы описывать всю функцию одной формулой (например, полиномом),
# B-сплайны используют комбинацию локальных полиномов, определённых на подотрезках оси X,
# соединённых в "узлах" (knots) - will be vertical dash lines, .
...
plot(basis)
# each curve - one of the basis, 
# (I don't know why, but it use same colors for different basis functions)
...
# best lambda
functionalParbest <- fdPar(fdobj=basis, Lfdobj=4, lambda=lambda[which.min(gcv)])  
...
dfbest # where we have to put it?
...
bw <- 0.1 # bandwidth (size of window, smaller window - smaller bias, higher variance)
...
bw <- 0.5 #larger bandwidth - larger bias, lower variance
...
# parameters should be same for all dimensions, because we all in one...
