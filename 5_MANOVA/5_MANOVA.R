...
# i.i.d - why we suppose that and never check
...
# picture also depend on observations, so be careful it can change
...
# confidence interval doest contain 0, so this two groups has difference 
# so to find where is difference - we have to check pairs of groups
...
# so we can see all differences 
# red and pink - has less difference and the probably has common means
# so it's about rejecting. 
# if we collect more data - may lead to other results, especially in extreme cases
# gree and yellow - maybe if we add more data, we will reject H0

# Be careful to apply transitivity in the statistical context!!
# {A not significantly (stat.) different from B} [A = B]   (red vs cyan (бирюзовый))
#                       and
...
# (not including the Bonferroni correction) - so without Bonferroni correction - it's wrong
# we use bonferroni to increase evidence when use multiple tests
Auni <- matrix(0,6,6)
for(i in 1:6) {
  for(j in i:6) {
...
# Bonf - family wise error (N test, want controll making one type 1 error among all test)
# we do whole procedure no more 5% to make one type error (95 per to no make 1 type error)
# Bengamin - false discovery rate (much softer way to control type 1 error among all test)

# Bonferroni correction - so with Bonferroni it's more complicated to reject H0 for each pair
# because it saving total confidence on 0.95 => as result confidence of each variable more than 0.95
...
# 1)  normality (multivariate) in each group (3 tests) # multiple features -> we use MVN test, shapiro was for one dimensions N1
Ps <- NULL
for(i in 1:g) {
...
## Two-ways ANOVA --------------------------------------------------------------------------------
#
# Gas stations dataset (p=1, g=2, b=2)
#
...
#        /!\ It is NOT equivalent to say that there is no effect of gas station
#       neither gasoline type on performances /!\
...
## Two-ways MANOVA -----------------------------------------------------------------------------
#
# Example 6.13 JW, p.319 (p=3, g=2, b=2)
#
...
# X.ijk = mu + tau.i + beta.j + eps.ijk; eps.ijk ~ N(0,sigma^2), 
#     i=1,2 (effect gender), j=1,2,3 (effect species)
fit2 <- aov(weight ~ gender + species) 
summary(fit2)

# Reduced model (one-way): 
# X.jk = mu + beta.j + eps.jk; eps.jk ~ N(0,sigma^2), 
#     j=1,2,3 (effect species)
fit3 <- aov(weight ~ species) 
summary(fit3)
...
# 1) normality
Ps <- c(shapiro.test(t(weight[species==levels(species)[1]]))$p,
        shapiro.test(t(weight[species==levels(species)[2]]))$p,
        shapiro.test(t(weight[species==levels(species)[3]]))$p)
Ps
...
### question b)
DF <- fit3$df # (b*n)*g - g = (2*15)*3 - 3 = 90 - 3 = 87
