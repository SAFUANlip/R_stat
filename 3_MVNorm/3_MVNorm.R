...
# H0 - is fact that our sample is Gaussian (so we don't want to regect H0)
...
result$multivariateNormality
# compute distance between our samples and mean => it have to be from Cho-Square distribution
# Malahnnobius distance have to be 8, but it 10, so this sample far from the mean
...
# to be Gaussian - at least features have to be gaussian
...
# MVN - multivariant normality

# The data don't seem Gaussian. What can we do?
# Identify clusters - and analysie each cluster separatly
# Identify (and possibly remove) outliers
# Transform the data (e.g., Box-Cox transformations) adress more squeness aspects 
...
# For lambda<1: observations <1 are "spread" (разбросанными), observations >1 are "shrinked" (сжатыми)
# For lambda>1: observations <1 are "shrinked", observations >1 are "spread"
# small labda - compress large values
# high labda - extend large values
...
set.seed(20032023)
x <- rnorm(n) ^ 2 - 10
x <- x + abs(min(x)) + 1 # we have too shift before transformation 
...
lambda.x_yj <- powerTransform(x, family = "yjPower")
lambda.x_yj$lambda

yj.x <- yjPower(x, lambda.x_yj$lambda)
hist(
  yj.x,
  col = 'grey',
  prob = T,
...
powerTransform(b) # same result as cbind(x, y)
lambda <- powerTransform(cbind(x, y))
lambda
# lambda[1] close to 1 - so just linear transform (we may not apply, because doesn't change)
...
# inside 
# accepted region - to du test
# confidence region - to evaluate our test
# if one of them inside - other also inside
# if one outside - other also outside
...
# red - mean of sample
# blue - real mean
# confidence region which show that in 95% of time will contain expected mean
