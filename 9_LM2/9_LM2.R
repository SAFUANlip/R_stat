...
# but PCA have to be applied on standartised variabel
summary(speed.pc)
speed.pc$load
...
summary(fm.pc) # now we see, that intercept important and sp1.pc 
...
#  = b0 + b1*(e11*(speed1-m1)+e21*(speed2-m2)) + b2*(e12*(speed1-m1)+e22*(speed2-m2)) + eps =
#  = b0 - b1*e11*m1 - b2*e12*m1 - b1*e21*m2 - b2*e22*m2 + 
#                           + (b1*e11+b2*e12)*speed1 + (b1*e21+b2*e22)*speed2 + eps
...
#          = b0 + b1*(e11*(speed1-m1)1+e21*(speed2-m2)) + eps =
#          = b0 - b1*e11*m1 - b2*e21*m2 + b1*e11*speed1 + b1*e21*speed2 + eps
beta0 <- coefficients(fm.pc)[1] - 
  coefficients(fm.pc)[2]*speed.pc$load[1,1]*m1 - 
  coefficients(fm.pc)[2]*speed.pc$load[2,1]*m2 
beta1 <- coefficients(fm.pc)[2]*speed.pc$load[1,1]
...
# take variabels, having high correlation with target 

# Otherwise, another possible solution to colinearity: ridge regression
 
### Ridge regression ---------------------------------------------------------------------------
help(lm.ridge) # penalise with L2 norm
# with ridge or lasso - we will not have confidence intervals (will not be able check on normality?)
...
lambda.c <- seq(0,10,0.01) # witl lambda = 0, we will have same lambda as in standart Linear model
...
# GCV -> Generalized Cross Validation # approximation of leave one out cross validation, but it's not a real cross validation
select(fit.ridge)
...
optlam.lasso 
# second vertical line - point out Lambda, such stat average MSE still one standart deviation from best lambda
# so it's better penalise more, to have better robustness 
...
summary(result2)
# reduce number of variables, while all will be significant
...
shapiro.test(residuals(result2)) # our residuals have to be normalliy distributed 
...
summary(regfit.full)
# table of best subset variabels 
# one variabels - CRBI
# 2 varaibels - Hits, CRBI
# 3 variabels - Hits, CRBI, PutOuts
