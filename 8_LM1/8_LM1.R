...
# Pr(> |t|) - p-value of each variable independetly H0 - betta = 0, H1 betta != 0
# Adjusted R-square (penalise for number of regressors)
...
# red line contain 95% only for one observation (doesn't include residuals)
# gold line - contain 95% of predictions
...
# Residuals vs Fitted - residuals mean have to be zero (red line)
# No change of variability along variabels (heteroginity) 
# (одинаковый варианц, не должно быть меняющегося variance вдоль values)
# BUT will be better if we also plot residual among each variabl, maybe residuals vs speed will have
# different variance (heteroginity for residuals vs variabels)

# Standardized residuals - ideally plot, on which we have to look
...
# If point above/under has high cook distance (above/under 1) then it probably outlier

shapiro.test(residuals(fm)) # checking normality of residuals 
...
abline(h=0)
# model should be quadratic

plot(x3,y3, main='Dataset 3')
abline(lm3)
# here we have very high cook distance (outlier in terms of feature)
...
abline(lm4)
# here we see outlier in terms of y and x
...
par(mfrow=c(2,2))
plot(result) # here we can see outliers abouve Cook line
...
# use only if we have classes and we want to solve regression task 
dummy <- clusterw - 1   # 0 = red
                        # 1 = green

Qd <- cbind(Q, dummy) # dummy just binary variabels 
head(Qd)
...
# first order - high bias, 
# last orders - high variance
...
## Example: Multiple linear regression with qualitative predictors ----------------------------
